...
class Solution {
    public List<List<Integer>> levelOrder(TreeNode root) {
        ...
        while(q.size()>0){
            Pair temp = q.remove();
            // if (curr== temp.second){
            //     subAns.add(temp.first.val);  // ans.get(curr).add(temp.first.val);
            // }
            // else {
            //     // ans.add(new ArrayList<>(subAns));
            //     // subAns.clear();
            //     curr = temp.second;
            //     subAns.add(temp.first.val); // ans.get
            // }
            if (ans.size()!=temp.second){
                // curr = temp.second;
                ans.add(new ArrayList<>(List.of(temp.first.val)));

            }
            else{
                ans.get(ans.size()-1).add(temp.first.val);
            }
